...
public abstract class SignatureData extends SignatureData_Base {
	...
	/**
	 * establishes a bidirectional realtion between this object and the {@link #genericSourceOfInfoBean}. And extracts its
	 * intention and
	 * description
	 * 
	 * @param dataBean
	 ...
	 * checks to see if it has an {@link MultipleObjectSignatureDataAggregator} associated, if it does, removes it
	 * 
	...
	public final Signature generateSignature(SignatureFormat signatureFormat, User signer, Set<User> permittedUsers) {
		...
		try {
			Signature sigToReturn =
					new Signature(signatureFormat, Base64.decode(getAuxiliarySignatureContentString()), this, signer);
			...
			if (this instanceof ObjectSignatureData && !(this instanceof MultipleObjectSignatureDataAggregator)) {
				MultipleObjectSignatureDataAggregator aggregator =
						((ObjectSignatureData) this).getMultipleObjectSignatureDataObject();
				if (aggregator != null) {
